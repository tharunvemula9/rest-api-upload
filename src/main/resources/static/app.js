var app = angular.module('apiuploadApp', []);
...
app.service('DocumentService', [ '$http', '$rootScope', function($http, $rootScope) {
	this.search = function(name, date) {
		$http.get(baseUrl + apiupload + "/documents", {
			params : {
				person : name,
				date : date
			}
...
app.controller('UploadCtrl', [ '$scope', 'fileUpload',
		function($scope, fileUpload) {
			$scope.uploadFile = function() {
				...
				var uploadUrl = baseUrl + apiupload + "/upload";
				fileUpload.uploadFileToUrl(uploadUrl, file, name, date);
			};
		} ]);

app.controller('apiuploadCtrl', function($scope, $http) {
...
app.run(function($rootScope, $http) {
	$http.get(baseUrl + apiupload + "/documents").success(
			function(response) {
				$rootScope.metadataList = response;
			});
});
